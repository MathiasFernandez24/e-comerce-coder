    ...
    useEffect(() => {
        if (location?.lat) {
            (async () => {
                ...
                setPhoto(`https://maps.googleapis.com/maps/api/staticmap?center=${location.lat},${location.lng}&zoom=15&size=600x600&maptype=roadmap&markers=color:blue%7Clabel:I%7C${location.lat},${location.lng}&key=${API_KEY}`)
                //REVERSE GEOCODE
